...
pub async fn create_test_app() -> Result<Router, Box<dyn std::error::Error + Send + Sync>> {
    use auth::{
        api_login, api_register, login_handler, login_page, logout, refresh_token,
    ...
    let mut tera = tera.ok_or_else(|| {
        ...
        format!(
            "Failed to load templates from any path. Current dir: {:?}",
            current_dir
        )
    })?;
    ...
    let app = Router::new()
        ...
        .route(
            "/htmx/user/communities-options",
            get(htmx::user_communities_options),
        )
        ...
        .route(
            "/api/users/dismiss-profile-banner",
            post(api::dismiss_profile_banner),
        )
