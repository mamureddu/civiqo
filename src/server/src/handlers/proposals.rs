...
pub async fn list_proposals(
    ...
    let response: Vec<ProposalResponse> = proposals.iter().map(|row| {
        ProposalResponse {
            ...
            voting_starts_at: row.get::<Option<chrono::DateTime<chrono::Utc>>, _>("voting_starts_at")
...
pub async fn create_proposal(
    ...
    let voting_starts: Option<chrono::DateTime<chrono::Utc>> = payload.voting_starts_at
        .as_ref()
        .and_then(|s| chrono::DateTime::parse_from_rfc3339(s).ok())
        .map(|dt| dt.with_timezone(&chrono::Utc));
    
    let voting_ends: Option<chrono::DateTime<chrono::Utc>> = payload.voting_ends_at
        .as_ref()
        .and_then(|s| chrono::DateTime::parse_from_rfc3339(s).ok())
        .map(|dt| dt.with_timezone(&chrono::Utc));
...
pub async fn get_results(
    ...
    // Get proposal info
    let proposal = sqlx::query(
        "SELECT quorum_required, community_id FROM proposals WHERE id = $1"
    )
    .bind(proposal_id)
    .fetch_optional(&state.db.pool)
    .await
    ...
    let results: Vec<VoteOption> = votes.iter().map(|row| {
        let count: i64 = row.get("count");
        VoteOption {
            value: row.get::<String, _>("vote_value"),
            count,
            percentage: if total_votes > 0 { (count as f64 / total_votes as f64) * 100.0 } else { 0.0 },
        }
    }).collect();
...
pub async fn get_results_fragment(
    ...
    html.push_str(&format!(
        ...
        data.total_votes,
        if data.total_votes > 0 { (data.total_votes as f64 / 100.0) * 100.0 } else { 0.0 }
    ));
    
    for option in &data.results {
        let color = if option.value == "yes" { "bg-civiqo-green" } 
                   else if option.value == "no" { "bg-red-500" } 
                   else { "bg-civiqo-blue" };
...
pub async fn close_proposal(
    ...
    let proposal = sqlx::query(
        "SELECT created_by, community_id, status FROM proposals WHERE id = $1"
    )
    .bind(proposal_id)
    .fetch_optional(&state.db.pool)
    .await
