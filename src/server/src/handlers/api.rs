...
pub async fn create_community(
    ...
    // Insert community (communities.id is BIGINT, auto-generated by database)
    ...
    let (community_id, community_row) = match community_result {
        Ok(row) => {
    ...
    // Get admin role ID (subquery approach)
    let admin_role_result: Result<Uuid, _> = sqlx::query_scalar(
