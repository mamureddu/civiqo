...
mod posts_integration_tests {
    ...
    async fn test_create_post_in_database() {
        ...
        // Verify post exists
        let count: i64 =
            sqlx::query_scalar!("SELECT COUNT(*) as count FROM posts WHERE id = $1", post_id)
                .fetch_one(&db.pool)
                .await
                .unwrap()
                .unwrap_or(0);
    ...
    async fn test_comment_threading() {
        ...
        // Verify threading
        let reply_parent: Option<Uuid> =
            sqlx::query_scalar!("SELECT parent_id FROM comments WHERE id = $1", reply_id)
                .fetch_one(&db.pool)
                .await
                .unwrap();
    ...
    async fn test_update_comment_sets_edited_flag() {
        ...
        let is_edited: Option<bool> =
            sqlx::query_scalar!("SELECT is_edited FROM comments WHERE id = $1", comment_id)
                .fetch_one(&db.pool)
                .await
                .unwrap();
    ...
    async fn test_reaction_counts() {
        ...
        // Cleanup
        let _ = sqlx::query!("DELETE FROM users WHERE id = $1", user2_id)
            .execute(&db.pool)
            .await;
        cleanup(&db, user_id, community_id).await;
    }
    ...
    async fn test_increment_view_count() {
        ...
        let initial_count: Option<i64> =
            sqlx::query_scalar!("SELECT view_count FROM posts WHERE id = $1", post_id)
                .fetch_one(&db.pool)
                .await
                .unwrap();
