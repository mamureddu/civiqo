...
static TEST_RUN_ID: LazyLock<String> =
    LazyLock::new(|| format!("__test_runner_{}", Uuid::now_v7()));

/// Create a test server instance with real database
async fn create_server() -> TestServer {
    ensure_setup_completed().await;
...
/// Ensure instance_settings has setup_completed so the homepage doesn't redirect to /setup
...
async fn cleanup_test_data(db: &Database) {
    ...
    sqlx::query!("DELETE FROM communities WHERE slug LIKE $1", prefix_pattern)
        .execute(&db.pool)
        .await
        .ok();
    ...
    sqlx::query!("DELETE FROM users WHERE email LIKE $1", test_email_pattern)
        .execute(&db.pool)
        .await
        .ok();
}
...
async fn test_view_interaction_01_homepage_loads() {
    ...
    assert!(
        status.is_success(),
        "Homepage should load successfully, got {}",
        status
    );
...
async fn test_view_interaction_01b_recent_communities_fragment() {
    ...
    assert!(
        ...
        "Should be fragment, not full page"
    );
...
/// Test #4: Communities page loads or redirects (single-community mode may redirect)
#[tokio::test]
async fn test_view_interaction_04_communities_page_loads() {
    ...
    let status = response.status_code();
    // In single-community mode, /communities may redirect to the community home
    assert!(
        status.is_success() || status.is_redirection(),
        "Communities page should load or redirect, got {}",
        status
    );
}
...
async fn test_view_interaction_06b_community_dynamic_stats() {
    ...
    // Add 3 members to the community
    for i in 0..3 {
    ...
    assert!(
        body.contains("Stats Test Community"),
...
async fn test_view_interaction_06c_community_feed_shows_posts() {
    ...
    let server = create_server().await;
    let response = server
        .get(&format!("/htmx/communities/{}/feed", community_id))
        .await;
    ...
    assert!(
        body.contains(&unique_title),
        "Feed should contain the post title: {}",
        unique_title
    );
...
async fn test_view_interaction_06d_communities_list_shows_data() {
    ...
    assert!(
        body.contains(&unique_name),
        "Communities list should contain: {}",
        unique_name
    );
...
async fn test_view_interaction_06e_index_recent_communities() {
    ...
    assert!(
        body.contains(&unique_name),
        "Recent communities should contain: {}",
        unique_name
    );
...
async fn test_view_interaction_06f_members_list_shows_data() {
    ...
    assert!(
        body.contains("\"total\":4") || body.contains("\"total\": 4"),
        "Members API should return total: 4, got: {}",
        body
    );
...
async fn test_view_interaction_06g_post_detail_shows_data() {
    ...
    assert!(
        body.contains(&unique_title),
        "Post detail should show title: {}",
        unique_title
    );
    // Verify comment count (2)
    assert!(
        body.contains("(2)") || body.contains("Commenti (2)"),
...
async fn test_view_interaction_07_09_community_posts_sorting() {
    ...
    // Test newest sort
    let response = server
        .get(&format!("/communities/{}/posts?sort=newest", community_id))
        .await;
    let status = response.status_code();
    ...
    // Test popular sort
    let response = server
        .get(&format!("/communities/{}/posts?sort=popular", community_id))
        .await;
    let status = response.status_code();
    assert!(status.is_success() || status.as_u16() == 404);
    ...
    let response = server
        .get(&format!(
            "/communities/{}/posts?sort=discussed",
            community_id
        ))
        .await;
    let status = response.status_code();
...
async fn test_view_interaction_10_create_community_page() {
    ...
    if status.is_success() {
        ...
        assert!(
            body.contains("hx-post") || body.contains("form"),
...
async fn test_view_interaction_27_28_members_list() {
    ...
    let exists = sqlx::query!("SELECT id FROM communities WHERE id = $1", community_id)
        .fetch_optional(&db.pool)
        .await
    ...
    assert!(
        exists.is_some(),
        "Community {} (slug: {}) should exist in DB",
        community_id,
        slug
    );
    ...
    if !status.is_success() {
        let body = response.text();
        eprintln!(
            "Members list failed for community {}: {} - {}",
            community_id, status, body
        );
    }
    assert!(
        status.is_success(),
        "Members list should succeed for existing public community {}",
        community_id
    );
...
async fn test_view_interaction_24_26_admin_operations_require_auth() {
    ...
    let response = server
        .post(&format!(
            "/api/communities/{}/promote/{}",
            community_id, fake_user_id
        ))
        .await;
    response.assert_status_unauthorized();
...
async fn test_view_interaction_dashboard_page_requires_auth() {
    ...
    let status = response.status_code();
    assert!(
        status.is_redirection() || status.as_u16() == 401,
        "Dashboard should require authentication, got {}",
        status
    );
}
...
async fn test_view_interaction_chat_page() {
    ...
    let response = server
        .get("/chat/00000000-0000-0000-0000-000000000001")
        .await;
...
async fn test_view_nav_fragment() {
    ...
    assert!(
        body.contains("<") && body.contains("href"),
        "Should be HTML with links"
    );
}
...
async fn test_htmx_business_reviews() {
    let server = create_server().await;
    let response = server
        .get("/htmx/businesses/test-business-id/reviews")
        .await;
...
async fn test_htmx_comment_reply_form() {
    let server = create_server().await;
    let response = server
        .get("/htmx/comments/test-comment-id/reply-form")
        .await;
...
/// Test: Navbar has link to Governance (single-community mode)
#[tokio::test]
async fn test_nav_navbar_has_governance_link() {
    let server = create_server().await;
    let response = server.get("/").await;
    response.assert_status_success();
    let body = response.text();
    ...
    assert!(
        body.contains("href=\"/governance\"") || body.contains("href='/governance'"),
        "Navbar MUST contain link to /governance"
    );
}
...
async fn test_nav_navbar_no_global_businesses() {
    ...
    assert!(
        !nav_section.contains("href=\"/businesses\"")
            && !nav_section.contains("href='/businesses'"),
...
async fn test_nav_community_has_governance_tab() {
    ...
    assert!(
        body.contains("Votazioni")
            || body.contains("governance")
            || body.contains("Governance")
            || body.contains("Proposte"),
...
async fn test_nav_community_has_businesses_tab() {
    ...
    assert!(
        body.contains("Attività")
            || body.contains("businesses")
            || body.contains("Negozi")
            || body.contains("Locali"),
...
async fn test_nav_htmx_community_businesses_endpoint() {
    ...
    let server = create_server().await;
    let response = server
        .get(&format!("/htmx/communities/{}/businesses", community_id))
        .await;
...
async fn test_nav_htmx_community_chat_endpoint() {
    ...
    let server = create_server().await;
    let response = server
        .get(&format!("/htmx/communities/{}/chat", community_id))
        .await;
...
async fn test_nav_community_has_join_button() {
    ...
    assert!(
        body.contains("Unisciti")
            || body.contains("join")
            || body.contains("Lascia")
            || body.contains("leave")
            || body.contains("Membro")
            || body.contains("Accedi per unirti"),
...
async fn test_nav_dashboard_page_exists() {
    ...
    let status = response.status_code();
    assert!(
        status.is_success() || status.is_redirection() || status.as_u16() == 401,
        "Dashboard page MUST exist (may require auth), got {}",
        status
    );
}
...
async fn test_nav_community_create_post_route_exists() {
    ...
    let server = create_server().await;
    let response = server
        .get(&format!("/communities/{}/posts/new", community_id))
        .await;
...
async fn test_nav_community_has_new_proposal_button() {
    ...
    assert!(
        body.contains("Nuova Proposta")
            || body.contains("proposal")
            || body.contains("Crea Proposta")
            || body.contains("New Proposal"),
...
async fn test_nav_notifications_page_exists() {
    ...
    assert!(
        status.is_success() || status.is_redirection() || status.as_u16() == 401,
        "Notifications page MUST exist (may require auth), got {}",
        status
    );
}
...
async fn test_view_interaction_zz_cleanup() {
    ...
    assert_eq!(
        ...
        0,
        "All test communities from this run should be deleted"
    );

    println!(
        "✅ Test data cleanup completed successfully for run: {}",
        &TEST_RUN_ID[15..23]
    );
}
